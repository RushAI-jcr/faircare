...
def _has_confidence_intervals(results: AuditResults) -> bool:
    ...
    return bool(disc.get("auroc_ci_95"))
...
def generate_raic_checkpoint_1_checklist(results: AuditResults, path: str | Path) -> Path:
    ...
    criteria: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "evidence": "Calibration by group available" if _has_subgroup_calibration(results) else "Overall only",
        },
        {
            "id": "AC1.CR90",
            "ls_id": "LS2.ME15",
        ...
        },
        {
            ...
            "evidence": "Subgroup performance available" if _has_subgroup_analysis(results) else "Missing",
        },
        {
            "id": "AC1.CR92",
            "ls_id": "LS2.ME37",
            ...
            "status": _format_status(_has_subgroup_calibration(results), else_status="PARTIAL"),
            "evidence": "Calibration differences computed" if _has_subgroup_calibration(results) else "Not computed",
        },
        {
            "id": "AC1.CR93",
            "ls_id": "LS2.ME38",
            ...
            "status": _format_status(
                bool(results.config.primary_fairness_metric and results.config.fairness_justification)
            ),
        ...
        },
        {
            ...
            "status": _format_status(_has_subgroup_analysis(results)),
            "evidence": "Subgroup fairness metrics available" if _has_subgroup_analysis(results) else "Missing",
        },
    ]
